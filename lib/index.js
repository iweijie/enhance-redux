...
export default function enhanceRedux(models = [], options = {}) {
      ...
      return effect(
        {
          ...
          getState: (namespace) => {
            ...
            try {
              ...
              console.warn(`getState：${namespace}取值错误，请查看取值路径是否正确`);
            }
            return state;
          },
          state: state[namespace],
  ...
  function defaultReduce(state = {}, action = {}) {
    ...
    // model 新增 or  移除
    if (type === IS_ENHANCE_REDUX_MODEL_ADDORREMOVE) return payload;
    try {
      const typeList = type.split(separator);
      const typeLastAttr = typeList.pop();
      const selected = typeList.reduce((obj, attr) => obj[attr], state);
